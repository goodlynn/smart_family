...
const NewMyTask = ({ onClose, onAddTask }) => {
  const [taskName, setTaskName] = useState('');
  const [description, setDescription] = useState('');
  const [priority, setPriority] = useState('low');
  const [isRecurring, setIsRecurring] = useState(false);
  const [selectedIcon, setSelectedIcon] = useState(icon1); // Устанавливаем первую иконку по умолчанию
            ...
            >
              <img 
                ...
                onError={(e) => {
                  ...
                  e.target.src = ''; // Можно указать fallback-изображение
                }}
              />
            </div>
          ))}
